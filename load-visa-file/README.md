...
To add the Visa item to a menu shared with other libraries, create the menu once and call `addToUi()` after adding every item.
...
function onOpen()
{
	...
	menu = GASLibLoadVisaFile.addItemToMenu(menu, "visa");
	menu.addToUi();
}
```
