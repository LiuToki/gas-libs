...
All item is under the GASLibLoadJREFile namespace.

## addMenuToMenuBar
	GASLibLoadJREFile.addMenuToMenuBar(menuName, itemName);
...
## addItemToMenu
	menu = GASLibLoadJREFile.addItemToMenu(menu, itemName);

### Description
Add a JRE item to an existing menu builder. Use the returned menu for subsequent additions, then call `addToUi()`.
...
### Returns
The menu builder with the JRE item added.

## callbackJRE
	GASLibLoadJREFile.callbackJRE(formObject, colNum, colDate, colName, colAmount, colRemarks, colCard, colOne);
