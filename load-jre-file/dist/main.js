var GASLibLoadJREFileBundle = (function (exports) {
    'use strict';
    ...
    function addMenuToMenuBarAux_(menuName, itemName) {
        ...
        menu.addItem(itemName, "GASLibLoadJREFile.jre");
        menu.addToUi();
    }
    /**
    ...
    function addItemToMenuAux_(menu, itemName) {
        return menu.addItem(itemName, "GASLibLoadJREFile.jre");
    }
    /**
     * Open JRE Dialog.
     */
    function jreAux_() {
        var html = HtmlService.createHtmlOutputFromFile("formJRE");
        SpreadsheetApp.getUi().showModalDialog(html, "load JRE file");
    }
    /**
     ...
     */
    function callbackJREAux_(formObject, colNum, colDate, colName, colAmount, colRemarks, colCard, colOne) {
        // 同じカラム番号は入れちゃダメ.
        // 適当過ぎてちょっとアレなコード.
        var isError = false;
                ...
                if (month == targetMonth) {
                    ...
                    for (var colIdx = 0; colIdx < colNum; ++colIdx) {
                        if (colIdx == colDate) {
                            ...
                            appendArray.push(textLinesCells[1]);
                        }
                        else if (colIdx == colAmount) {
                            appendArray.push(textLinesCells[5]);
                        }
                        else if (colIdx == colRemarks) {
                            appendArray.push(textLinesCells[6]);
                        }
    ...
    exports.callbackJRE = callbackJRE;
    exports.jre = jre;
